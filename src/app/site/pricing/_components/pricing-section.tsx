...
interface PricingSectionProps {
    ...
    user: {
        ...
        agencies?: { id: string; name: string; isPrimary?: boolean }[]
    } | null
}
...
export function PricingSection({
    ...
    const defaultAgencyId = agencies.find((a) => a.isPrimary)?.id ?? agencies[0]?.id ?? null
    const [selectedAgencyId, setSelectedAgencyId] = useState<string | null>(defaultAgencyId)
    const [pendingCheckout, setPendingCheckout] = useState<{ priceId: string; quantity?: number } | null>(null)
    ...
    const goToCheckout = (priceId: string, quantity?: number) => {
        const query = quantity ? `?quantity=${quantity}` : ''

        // Logged out or no agencies yet -> create-new flow via /site
        if (!user || agencies.length === 0) {
            router.push(`/site/pricing/checkout/${priceId}${query}`)
            return
        }

        // Logged in + has agencies -> ask where to apply the subscription
        setPendingCheckout({ priceId, quantity })
        setSelectedAgencyId(defaultAgencyId)
        setAgencySelectorOpen(true)
    }

    const proceedWithSelectedAgency = () => {
        if (!pendingCheckout?.priceId) return
        if (!selectedAgencyId) return

        const q = new URLSearchParams({ plan: pendingCheckout.priceId })
        if (pendingCheckout.quantity) q.set('quantity', String(pendingCheckout.quantity))

        // Best practice: Existing customers manage subscriptions inside the product context.
        router.push(`/agency/${selectedAgencyId}/billing/checkout?${q.toString()}`)
        setAgencySelectorOpen(false)
    }

    const proceedCreateNewAgency = () => {
        if (!pendingCheckout?.priceId) return
        const query = pendingCheckout.quantity ? `?quantity=${pendingCheckout.quantity}` : ''
        router.push(`/site/pricing/checkout/${pendingCheckout.priceId}${query}`)
        setAgencySelectorOpen(false)
    }
    ...
    return (
        <>
                            ...
                            >
                                ...
                                <CardFooter className="relative px-5 pb-5 pt-4">
                                    {card.key === 'ENTERPRISE' ? (
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => goToCheckout(card.priceId)}
                                            className={clsx(
                                                "group/cta w-full text-center rounded-md font-medium text-sm transition-all duration-300",
                                                "h-9 inline-flex items-center justify-center gap-1",
                                                "focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-surface-primary",
            ...
            {/* Agency Selector (when user already has agencies) */}
            <Dialog open={agencySelectorOpen} onOpenChange={setAgencySelectorOpen}>
                <DialogContent className="sm:max-w-lg">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-content-secondary">
                            Apply this checkout to an existing agency, or create a new one.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-3 py-2">
                                ...
                                return (
                                    <button
                                        ...
                                        className={clsx(
                                            ...
                                            selected
                                                ? "border-brand-bg/60 ring-1 ring-brand-bg/20"
                                                : "border-line-primary"
                                        )}
                                    >
                                        <div className="flex items-center justify-between gap-3">
                                            <div className="flex items-center gap-2">
                                                <div className={clsx(
                                                    ...
                                                    selected ? "bg-brand-bg/15" : "bg-surface-secondary"
                                                )}>
                                                    <Building2 className={clsx("h-4 w-4", selected ? "text-brand-bg" : "text-content-secondary")} />
                                                </div>
                                                <div>
                                                    ...
                                                    <div className="text-xs text-content-tertiary">
                                                        Manage billing inside agency context
                                                    </div>
                                                </div>
                                            </div>
                                            {a.isPrimary ? (
                                                <div className="rounded-md border border-brand-bg/30 bg-brand-bg/10 px-2 py-1 text-[11px] font-medium text-brand-bg">
                                                    Primary
                                                </div>
                                            ) : null}
                                        </div>
                        ...
                        <div className="rounded-lg border border-line-secondary bg-surface-secondary px-4 py-3">
                            <div className="flex items-start gap-3">
                                <div className="flex h-8 w-8 items-center justify-center rounded-md bg-surface-tertiary">
                                    <Plus className="h-4 w-4 text-content-secondary" />
                                </div>
                                <div className="flex-1">
                                    <div className="text-sm font-semibold text-content-primary">Create a new agency</div>
                                    <div className="text-xs text-content-tertiary">
                                        Use the site checkout flow to create a new workspace and subscribe.
                                    </div>
                                </div>
                                <Button variant="outline" size="sm" onClick={proceedCreateNewAgency}>
                                    Create
                                </Button>
                            </div>
                        </div>
                    ...
                    <DialogFooter className="gap-2 sm:gap-3">
                        <Button variant="outline" onClick={() => setAgencySelectorOpen(false)}>
                            Cancel
                        </Button>
                        <Button
                            onClick={proceedWithSelectedAgency}
                        ...
                        <Button
                            onClick={() => {
                                if (selectedEnterpriseCard) {
                                    ...
                                    goToCheckout(selectedEnterpriseCard.priceId, subAccountCount)
                                }
                            }}
                            className="bg-brand-gradient hover:bg-brand-gradient-hover"
                        >
