...
interface ModularNavProps {
    ...
    triggerLabel?: string
    /** Starting hierarchy level - determines where navigation starts */
    from: HierarchyKind
    /** 
     * How many levels deep to show (REQUIRED)
     * - module: 1|2|3
     * - submodule: 2|3 
     * - feature: 1 only
     */
     ...
     */
    featureKey?: string
    /** Custom items override (skips auto-build from registry) */
    items?: NavItem[]
...
export const ModularNav = ({
    ...
    // Validate and clamp levels config
    ...
    }, [customItems, from, validatedLevels, resolvedKeys, baseHref])
                    ...
                    <DropdownMenuContent align="end" className="w-56">
                        {filteredNavItems.map((item, idx) => (
                            <React.Fragment key={item.key}>
                                ...
                                {idx < filteredNavItems.length - 1 && item.children && (
                                    <DropdownMenuSeparator />
                                )}
                            </React.Fragment>
                        ))}
