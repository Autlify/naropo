...
export function SupportWizardPanel({ scope }: { scope: SupportScope }) {
  ...
  const runDiagnostics = async () => {
    ...
    try {
      const parts: any = {}
      if (canRunBilling) {
        const r = await fetch(`${base}/diagnostics/billing?${qp}`, { headers: scopeHeaders, cache: 'no-store' })
        parts.billing = await safeJson(r)
      }
      if (canRunWebhooks) {
        const r = await fetch(`${base}/diagnostics/webhooks?${qp}`, { headers: scopeHeaders, cache: 'no-store' })
